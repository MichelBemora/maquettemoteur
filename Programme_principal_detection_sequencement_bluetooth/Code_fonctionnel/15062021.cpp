...
unsigned int value_modif_pMax;
unsigned int pMin_in_millivolt;
unsigned int pMax_in_millivolt;

//Exécution d'actionneurs
const int cmd_injection = 25;
const int cmd_avance_allumage= 26;
...
//VARIABLES DE CONDITIONS DETECTION DES DENTS ET CYCLE
bool top = 0;
boolean etat = false;
boolean cycle = 0;
int l;
...
//VARIABLE ACTIONNEUR-BOBINE
const int T_charge_bobine = 3000; // valeur fixe à 3 ms
int T_avance_bobine;

//VARIABLE ACTIONNEUR-INJECTION
...
const int T_ouverture_injection = 1700; //L'injecteur s'ouvre en 1700 µs (1.7 ms)
...
//=================================================================================
// ---------- PROGRAMME INTERRUPTION SUR LES FRONTS MONTANTS ----------------------
//=================================================================================
...
//=================================================================================
  ...
  //Détection des deux dents manquantes
...
//=================================================================================
  ...
  //===========ALLUMAGE===========//
    ...
    if( (micros() - chrono) >= (PMH - T_avance_bobine - T_charge_bobine) ) {
      //BOBINE CHARGE
      digitalWrite(cmd_avance_allumage, HIGH);
    }
    if( (micros() - chrono) >= (PMH - T_avance_bobine) ) {
      digitalWrite(cmd_avance_allumage, LOW);
      //BOBINE DECHARGE
  }
}
...
void setup() {
  ...
  pinMode(cmd_injection, OUTPUT);
  pinMode(cmd_avance_allumage, OUTPUT);
  pinMode(39, INPUT);
...
int calcul_l(unsigned int angle_pap_local) {
...
int calcul_c(unsigned int rpm_local) {
...
int recup_injection(unsigned int angle_pap_local, unsigned int rpm_local) {
...
int recup_allumage(unsigned int angle_pap_local, unsigned int rpm_local) {
  l = calcul_l(angle_pap_local);
  c = calcul_c(rpm_local);
...
//========================= Calculs =========================//
  ...
  ////Calcul angle PMH (de 0 à 168°) en temps (µs)////
  PMH = PMH_pos * 1000000 / (rpm * 6);

  ////Calcul angle de commande de l'injection (de 0 à 138°) en temps (µs)////
...
//================================Partie Bluetooth================================//
  ...
  //Envoie de données vers l'application Android par Bluetooth
...
//Réception de données par Bluetooth
  if (SerialBT.available()) {
    ...
    //////////////////////////////////////////////////////////////////////////////
    if (which_BT_command == 1 || which_BT_command == 2 || which_BT_command == 4 || which_BT_command == 5) {
    ...
    //Réinitialisation des paramètres d'injection et d'avance à l'allumage
    else if (which_BT_command == 3) {
      Serial.println("Paramètres moteur réinitialisés");
      Serial.println("--------------------------------------");
    ...
    //Modification de la tension minimale
    ...
    //Modification de la tension maximale
    ...
    //Plage de tension reset
    else if (which_BT_command == 6) {
      Serial.println("Plage de tension réinitialisée");
      Serial.println("--------------------------------------");
